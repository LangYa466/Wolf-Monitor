...
const zhHant: Dict = {
  ...
  sortNetSent: "累計上行",
  sortNetRecv: "累計下行",
  sortGroupIdentity: "屬性",
  sortGroupResource: "資源",
  sortGroupNetwork: "流量",
  sortDirDesc: "由大到小（點擊切換）",
  sortDirAsc: "由小到大（點擊切換）",
  sortDirZA: "Z → A（點擊切換）",
  sortDirAZ: "A → Z（點擊切換）",
  sortDirOnlineFirst: "在線優先（點擊切換）",
  sortDirOfflineFirst: "離線優先（點擊切換）",
  ...
  deleteNode: "刪除節點",
  countryOverride: "國旗",
  countryOverrideHint: "輸入 2 位 ISO 國家碼（如 cn、hk、us），留空 = 自動偵測。",
  countryAuto: "自動",
  ...
  agentVersionLabel: "節點目標版本（留空 = 不推送）",
  agentVersionHint: "設定後,所有節點下次 /api/tasks 拉取(30s內)會自動 install.sh 升級到此版本。版本不匹配 5 分鐘冷卻一次重試,失敗會留在原版。",
  save: "儲存",
  ...
  secHttp: "HTTP 檢測",
  secHttpDesc: "定時 HEAD/GET 目標網址，非 2xx 或無法連線時走 Telegram/Webhook 通知（僅在狀態翻轉時發，不重複）。",
  thServer: "伺服器",
  thEnabled: "啟用",
  thGrace: "寬限（秒）",
  thStatus: "狀態",
...
const en: Dict = {
  ...
  sortStatus: "Status",
  sortNetUp: "Live Up",
  sortNetDown: "Live Down",
  sortNetSent: "Total Up",
  sortNetRecv: "Total Down",
  sortGroupIdentity: "Identity",
  sortGroupResource: "Resource",
  sortGroupNetwork: "Network",
  sortDirDesc: "Largest first (click to flip)",
  sortDirAsc: "Smallest first (click to flip)",
  sortDirZA: "Z → A (click to flip)",
  sortDirAZ: "A → Z (click to flip)",
  sortDirOnlineFirst: "Online first (click to flip)",
  sortDirOfflineFirst: "Offline first (click to flip)",
  ...
  countryOverride: "Flag",
  countryOverrideHint: "2-letter ISO country code (e.g. cn, hk, us). Leave empty to auto-detect.",
  countryAuto: "Auto",
  ...
  ipinfoPh: "ipinfo token",
  agentVersionLabel: "Target node version (empty = don't push)",
  agentVersionHint: "When set, every node self-runs install.sh on its next /api/tasks poll (≤30s). Mismatched versions retry every 5 min on failure; nodes stay on the old binary if install fails.",
  save: "Save",
  ...
  secHttp: "HTTP checks",
  secHttpDesc: "Periodic HEAD/GET against target URLs; non-2xx or unreachable fires Telegram/webhook (dedup'd — one alert per state transition).",
  thServer: "Server",
  thEnabled: "Enabled",
  thGrace: "Grace (s)",
  thStatus: "Status",
